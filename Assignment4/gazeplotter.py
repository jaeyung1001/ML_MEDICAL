...
def draw_scanpath(fixations, saccades, dispsize, imagefile=None, alpha=0.5, savefilename=None):
	...
	if saccades:
		...
		for st, et, dur, sx, sy, ex, ey, slen, svel, sacc in saccades:
			# draw an arrow between every saccade start and ending
			ax.arrow(sx, sy, ex-sx, ey-sy, alpha=alpha, fc=COLS['aluminium'][0], ec=COLS['aluminium'][5], fill=True, shape='full', width=5, head_width=5, head_starts_at_zero=False, overhang=0)
...
def draw_blk(time, sblk, eblk, savefilename=None):

		# PLOT BLINKS over time
		# create a blink array
		# will hold a 1 for every timestamp where a blink was detected
		blk = []

		for i in range(len(blk)):

			# set first to no blink (overwritten in the following if it does contain a blink)
			blk.append(0)

			# write 1 where there is a blink
			if time[i] in sblk:
				# if the value :
				blk[i] = 1
				
			# if blink ended, write 0
			elif blk[i-1] == 1:
				# if the value before is a blink end
				#print time[i-1] ###PROBLEM: This causes time to replace all values by 1.0
				if time[i-1] in eblk:
					blk[i] = 0
				# if the value before is NOT a blink end
				else:
					blk[i] = 1
					#print "I get activated"
			# write 0 where there is no blink
			else:
				blk[i] = 0
				

		#print "blk_array = ", blk_array[50:70]
		pyplot.plot(time,blk)
		pyplot.xlabel('time[in s]')
		pyplot.ylabel('blinks')
		pyplot.ylim((0,2))
		pyplot.title('Distribution of Blinks over Time')

		# store each plot automatically
		if savefilename != None:
...
def draw_display(dispsize, imagefile=None):
	...
	ax.imshow(img, cmap="gray", extent=[0, dispsize[0], 0, dispsize[1]])
...
def parse_fixations(fixations):
	...
	for fixnr in range(len(fixations)):
		stime, etime, dur, ex, ey, disp = fixations[fixnr]
		fix['x'][fixnr] = ex
		fix['y'][fixnr] = ey
		fix['dur'][fixnr] = dur
